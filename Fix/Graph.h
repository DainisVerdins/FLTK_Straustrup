...
#undef minor
	...
	struct Arc : Shape {
		//\param[in] p, ww, h bounding box of complete circle
		//\param[in] a1, a2 start and end angles of arc measured in degrees counter - clockwise from 3 o'clock. \p a2 must be greater than or equal to \p a1.
		Arc(Point center_coords, int r, double start_angle, double end_angle);

		//bounding box

		double get_start_angle() const { return start; }
		double get_end_angle() const { return end; }
		void set_angles(double start_angle, double end_angle);
